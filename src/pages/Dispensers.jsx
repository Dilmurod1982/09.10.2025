...
const Dispensers = () => {
  ...
  const [typeDispensers, setTypeDispensers] = useState([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const typeDispensersSnapshot = await getDocs(
          collection(db, "typeofdispensers")
        );
        const typeDispensersData = typeDispensersSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
        setTypeDispensers(typeDispensersData);
  ...
  const filteredDispensers = dispensers.filter(
    (dispenser) =>
      ...
      dispenser.serialNumber?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  // Обработчик выбора типа колонки
  const handleTypeDispenserChange = (typeId) => {
    const selectedType = typeDispensers.find((type) => type.id === typeId);
    if (selectedType) {
      if (isCreating) {
        setNewDispenser((prev) => ({
          ...prev,
          typeDispenserId: typeId,
    ...
    try {
      if (isCreating) {
        await addDoc(collection(db, "dispensers"), {
          ...newDispenser,
          manufactureYear: parseInt(newDispenser.manufactureYear),
          createdAt: new Date(),
          updatedAt: new Date(),
        });
      } else {
        await updateDoc(doc(db, "dispensers", selectedDispenser.id), {
          ...selectedDispenser,
          manufactureYear: parseInt(selectedDispenser.manufactureYear),
          updatedAt: new Date(),
        });
      }
  ...
  const handleDelete = async (dispenserId) => {
    if (window.confirm("Вы уверены, что хотите удалить эту колонку?")) {
      try {
        ...
        setDispensers(dispensers.filter((disp) => disp.id !== dispenserId));
      } catch (error) {
  ...
  // Получение текущей станции колонки
  const getCurrentStation = (dispenser) => {
    if (!dispenser.movementHistory || dispenser.movementHistory.length === 0) {
      return "Не назначена";
    }

    const sortedHistory = dispenser.movementHistory
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 p-4 lg:p-8">
      ...
      <motion.div
        ...
        <div>
          ...
          <p className="text-gray-600">
            Управление колонками и их движением по станциям
          </p>
        </div>

        <motion.button
          className="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-6 py-3 rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300 flex items-center gap-2 w-full lg:w-auto justify-center"
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredDispensers.map((dispenser, index) => (
                <motion.tr
                  key={dispenser.id}
                  className="hover:bg-green-50 transition-colors duration-200 cursor-pointer group"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  ...
                  <td
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">
                        {dispenser.serialNumber || "Не указан"}
                      </span>
                    </div>
                  </td>
                  <td
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">
                        {dispenser.manufactureYear || "Не указан"}
                      </span>
                    </div>
                  </td>
                  <td
        ...
        {filteredDispensers.length === 0 && (
          <motion.div
            ...
            <h3 className="text-lg font-semibold text-gray-600 mb-2">
              {searchTerm ? "Колонки не найдены" : "Колонки не добавлены"}
            </h3>
            <p className="text-gray-500 mb-4">
              {searchTerm
                ? "Попробуйте изменить условия поиска"
                : "Начните с добавления первой колонки"}
            </p>
            {!searchTerm && (
              <button
                onClick={handleCreateDispenser}
              ...
              <div className="p-6 max-h-[60vh] overflow-y-auto">
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <div>
                        ...
                        <select
                          value={
                            isCreating
                              ? newDispenser.typeDispenserId
                              : selectedDispenser?.typeDispenserId || ""
                          }
                          onChange={(e) =>
                            handleTypeDispenserChange(e.target.value)
                          }
                          ...
                          <option value="">Выберите тип колонки</option>
                          {typeDispensers.map((type) => (
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={
                            isCreating
                              ? newDispenser.manufactureYear
                              : selectedDispenser?.manufactureYear || ""
                          }
                          onChange={(e) =>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <textarea
                          value={
                            isCreating
                              ? newDispenser.purchaseDocuments?.description
                              : selectedDispenser?.purchaseDocuments
                                  ?.description || ""
                          }
                          onChange={(e) =>
                        ...
                        {isCreating || isEditMode ? (
                          <div className="space-y-2">
                            <input
                              ...
                              className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all disabled:bg-gray-50"
                            />
                            {uploadingFile && (
                            ...
                            {(isCreating
                              ? newDispenser.purchaseDocuments?.fileUrl
                              : selectedDispenser?.purchaseDocuments
                                  ?.fileUrl) && (
                              <div className="flex items-center gap-2 text-green-600 text-sm">
                                <FileText size={16} />
                                ...
                                {isCreating
                                  ? newDispenser.purchaseDocuments.fileName
                                  : selectedDispenser.purchaseDocuments
                                      .fileName}
                              </div>
                            )}
                          ...
                          <div>
                            {selectedDispenser?.purchaseDocuments?.fileUrl ? (
                              <div className="flex items-center gap-2">
                                <FileText
                                ...
                                <span className="text-gray-700">
                                  {selectedDispenser.purchaseDocuments.fileName}
                                </span>
                                <a
                                  href={
                                    selectedDispenser.purchaseDocuments.fileUrl
                  ...
                  <div>
                    ...
                    {!isCreating &&
                    selectedDispenser?.movementHistory?.length > 0 ? (
                      <div className="space-y-3">
                        {selectedDispenser.movementHistory
                          .sort((a, b) => new Date(b.date) - new Date(a.date))
                          .map((move, index) => (
                            <motion.div
                              key={index}
              ...
              <div className="border-t px-6 py-4 bg-gray-50">
                <div className="flex flex-col sm:flex-row gap-3 justify-between items-center">
                  {!isCreating && !isEditMode && (
                    <motion.button
                      onClick={handleEdit}
                      className="w-full sm:w-auto bg-green-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-green-600 transition-colors flex items-center gap-2 justify-center"
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}>
                      <Edit size={16} />
                      Редактировать
