...
const HomeChief = () => {
    ...
    try {
      ...
      try {
        ...
        allReports = reportsSnapshot.docs
          ...
          .filter((report) => managedStations.includes(report.stationId));
      } catch (error) {}
  ...
  const analyzeMissingReports = async (reports, period) => {
    try {
      ...
      datesToCheck.forEach((date) => {
        ...
        const stationsMissing = managedStations
  ...
  const analyzeControlDifference = (reports, period) => {
    ...
    console.log(
      `Found ${problematicReports.length} problematic control sum reports`
    );
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-sm text-gray-500">
            Текущий пользователь: {userData?.email || "Неизвестно"}
          </div>
          <div className="text-sm text-gray-500">
            Поле stations: {JSON.stringify(managedStations)}
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-8 p-4 bg-gray-100 rounded-lg">
          <h3 className="font-semibold mb-2">Информация о данных:</h3>
          <div className="text-sm space-y-1">
            <div>Управляемые станции: {managedStations.length}</div>
            <div>Загружено отчетов: {analysisData.autopilotData.length}</div>
            <div>Пользователь: {userData?.email}</div>
            <div>Роль: {userData?.role}</div>
          </div>
        </div>
      </div>
    </div>
...
const ControlDifferenceDetails = ({ data, period, onPeriodChange }) => {
  ...
  const getDifferenceColor = (amount, controlAmount, difference) => {
    if (controlAmount === 0 && amount > 0) {
      ...
      return "text-yellow-500"; // Желтый - контрольная сумма меньше суммы отчета
    } else {
